...
Drive interactive CLI coding agents (pi, codex, REPLs) through per-run **private tmux sockets** and control-file driver scripts. This is the fallback for `/herdr-driver` — use it when Herdr is not installed or its server can't run. Same doctrine, different transport: nothing you `start`, `kill-server`, or break may share a socket with another live session.
...
- `/herdr-driver` is preferred; reach for this only when `herdr` is unavailable, or the seat is already wired to the tmux drive scripts (`~/.claude/pi-drive.sh`, `~/.claude/codex-drive.sh`).
